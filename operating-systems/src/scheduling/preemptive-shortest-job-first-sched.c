...
void stcf_scheduler(Job jobs[], int count) {
    ...
    while (1) {
        ...
        // Check for job arrivals and preemption
        for (int i = 0; i < count; i++) {
            if (jobs[i].arrival_time <= currentTime && jobs[i].remaining_time > 0 && jobs[i].remaining_time < minTime) {
